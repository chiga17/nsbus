...
def route_stops(mreza_id: str) -> list[dict]:
    ...
    for item in raw:
        ...
        if key in seen:
            continue
        seen.add(key)
        name = re.sub(r"\s+", " ", parts[3]).strip().title()
        stops.append({"name": name, "lat": float(parts[2]), "lon": float(parts[1])})
    return stops
...
def write_stop_orders(lines: list[dict]) -> None:
    """Export the reconstructed stop sequence separately from map geometry."""
    orders = []
    markdown = [
        "# Bus stop order",
        "",
    ...
    for line in lines:
        stops = [
            {
                ...
                "lat": stop["lat"],
                "lon": stop["lon"],
                "distanceFromStartMeters": stop["along"],
            }
            for index, stop in enumerate(line["stops"], start=1)
        ]
        orders.append(
            {
                "line": line["id"],
                "route": line["route"],
                "mrezaId": line["mrezaId"],
                "stops": stops,
            }
        )
        ...
        for stop in stops:
            name = re.sub(r"[\r\n]+", " ", stop["name"]).strip()
            markdown.append(
                f"| {stop['sequence']} | {name} | "
                f"{stop['distanceFromStartMeters']:.0f} m | "
                f"{stop['lat']:.6f}, {stop['lon']:.6f} |"
            )
        markdown.append("")

    STOP_ORDERS_PATH.write_text(
        json.dumps(orders, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    STOP_ORDERS_MD_PATH.write_text("\n".join(markdown), encoding="utf-8")
...
def main() -> None:
    ...
    write_stop_orders(lines)
    print(f"wrote {STOP_ORDERS_PATH.relative_to(ROOT)}")
    print(f"wrote {STOP_ORDERS_MD_PATH.relative_to(ROOT)}")
